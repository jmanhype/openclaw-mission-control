...
def test_lead_templates_include_allowlist_guidance_and_avoid_shell_line_continuations() -> None:
    ...
    assert "Before asking a human to choose a model or workflow, enumerate the viable target-machine lanes first" in rendered["AGENTS.md"]
    assert "Reject external-auth or interactive-browser-required lanes by default" in rendered["AGENTS.md"]
    assert "Wan local first, `linoyts` LTX when LTX is specifically needed" in rendered["AGENTS.md"]
    ...
    assert "Before asking a human to pick a model or workflow, enumerate the viable target-machine lanes first" in rendered["HEARTBEAT.md"]
    assert "prefer Wan local first, use `linoyts` LTX when LTX is specifically required" in rendered["HEARTBEAT.md"]
    assert "## Media lane selection policy" in rendered["TOOLS.md"]
    assert "Classify each candidate lane as local/no-auth, external/no-auth, or external-auth/manual." in rendered["TOOLS.md"]
    assert "default lane order is: Wan local first, `linoyts` LTX when LTX is specifically needed" in rendered["TOOLS.md"]
